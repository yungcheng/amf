...
trait ShapeModel extends DomainElementModel with LinkableElementModel with KeyField with DescriptionField {
  ...
  val DisplayName = Field(Str, Schema + "name", ModelDoc(ExternalModelVocabularies.SchemaOrg, "name" , "Name for a data shape"))

  val Default = Field(DataNodeModel, Shacl + "defaultValue", ModelDoc(ExternalModelVocabularies.Shacl, "default value", "Default value parsed for a data shape property"))

  // TODO: change namespace
  val DefaultValueString = Field(Str, Shacl + "defaultValueStr", ModelDoc(ExternalModelVocabularies.Shacl, "default value String", "Textual representation of the parsed default value for the shape property"))
  ...
  /**
    ...
    */
  val Inherits = Field(Array(ShapeModel), Shapes + "inherits", ModelDoc(ModelVocabularies.Shapes, "inherits", "Relationship of inheritance between data shapes"))

  // Logical constraints:

  val Or = Field(Array(ShapeModel), Shacl + "or", ModelDoc(ExternalModelVocabularies.Shacl, "or", "Logical or composition of data shapes"))

  val And = Field(Array(ShapeModel), Shacl + "and", ModelDoc(ExternalModelVocabularies.Shacl, "and", "Logical and composition of data shapes"))

  val Xone = Field(Array(ShapeModel), Shacl + "xone", ModelDoc(ExternalModelVocabularies.Shacl, "exclusive or", "Logical exclusive or composition of data shapes"))

  val Not = Field(ShapeModel, Shacl + "not", ModelDoc(ExternalModelVocabularies.Shacl, "not", "Logical not composition of data shapes"))

  override val key: Field = Name

  // RAML user-defined facets: definitions and values
  lazy val CustomShapePropertyDefinitions = Field(Array(PropertyShapeModel), Shapes + "customShapePropertyDefinitions", ModelDoc(ModelVocabularies.Shapes, "custom shape property definitions", "Custom constraint definitions added over a data shape"))
  lazy val CustomShapeProperties          = Field(Array(ShapeExtensionModel), Shapes + "customShapeProperties", ModelDoc(ModelVocabularies.Shapes, "custom shape properties", "Custom constraint values for a data shape"))
  //
